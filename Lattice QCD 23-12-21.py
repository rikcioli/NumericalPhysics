...
class LatticeQCD:
    ...
    def __init__(self, N_lat = 8, N_config = 50, N_cor = 50, eps = 0.24, u0to4 = 0.8):
        """
        ...
        eps : float, optional
            Parameter which controls the magnitude of the shift in the metropolis algorythm.
            The default is 0.24.
        ----------
        rng : numpy Generator
        ...
        acceptance : float
            Fraction of successful hits of the metropolis algorythm. Initialized to 0.
        """
        ...
        #initialize the random matrices to use for the metropolis algorythm
    ...
    def compute_random_matrices(self):
        """
        Computes a pool N_rnd_matrices random SU(3) matrices for the metropolis algorythm.
        ...
        for k in range(0, self.N_rnd_matrices, 2):
            ...
            M = Re_M + Im_M*1j
            """
            M = np.zeros([3,3], dtype = np.complex)             #initialize empty complex matrix
            for i in range(3):
                for j in range(3):
                    M[i][j] = rnd.uniform(-1, 1) + rnd.uniform(-1, 1)*1j
            """
    ...
    def ten_hits_metropolis_enhanced(self, i, j, k, l, mu):
        """
        Metropolis algorythm applied 10 times on each site, before moving on to the next.
        ...
        is stored in Gamma.
        The second part is the proper metropolis algorythm, using standard Wilson action.
        ...
        while nu != mu:                     
            ...
            #each link has 2 neighbour plaquettes, left and right
            for orientation in hand:           
                
                i_pos = np.array([i,j,k,l])         #starting point
                i_pos[mu] += 1                       #move along mu direction
                delta_1 = np.zeros(4, dtype=int)
                delta_1[mu] = 1                       #displacement vector
                path = [copy.copy(i_pos)]          #list to store the points needed to close the plaquette
            
                for links in range(3):          #a path of 3 points is required to close plaquette
                    for trial in hand:          #tries to move towards increasing or decreasing coordinates
                                               
                        #try to move in a direction orthogonal to delta_1
                        delta_2 = np.zeros(4, dtype=int)
                        if links == 1:
                            delta_2[mu] = trial
                        else:
                            delta_2[nu] = trial
                            
                        #evaluate handedness of the turn
                        handedness = linalg.det([delta_1[slots], delta_2[slots]])
                        
                        #accept the turn if handedness matches the required orientation
                        if handedness == orientation:
                            i_pos += delta_2
                            path.append(copy.copy(i_pos))
                            delta_1 = copy.copy(delta_2)
                            break
                        
                #at this point we have a left handed/right handed path of 3 links
    ...
    def thermalize(self):
        ...
        for therm_index in tqdm.tqdm(range(5*self.N_cor), desc = "Thermalizing the lattice"):
            ...
            #metropolis algorythm is called for each link
    ...
    def tadpole_improve(self):
        ...
        The adjustment is realized by an algorythm in which the new input is the
    ...
    def bootstrap_plaquettes(self):
        ...
        N_bootstrap times. Thus, it can be used to estimate the uncertainty
        on the mean by simply evaluating the SDOM.       
        ...
        ensemble_of_plaquettes, ensemble_of_rects : list of float
            Each element of the list is the average of the plaquette/rectangle operator
        ...
        err_plaquette, err_rect : float
            ...
            rectangle, computed as the SDOM.
        ...
        #average to obtain estimates for the plaquette and rectangle operator
        mean_plaquette, mean_rect = 0, 0
        for alpha in range(self.N_cf):
            mean_plaquette += arr_of_plaquettes[alpha]/self.N_cf
            mean_rect += arr_of_rects[alpha]/self.N_cf
            
        #initialize the lists of average values of plaquette/rectangle operator
        #over bootstrap copies
        ...
        #evaluate standard deviation of the ensembles
        err_plaquette, err_rect = 0, 0
        for i in range(N_bootstrap):
            err_plaquette += np.abs(ensemble_of_plaquettes[i]**2 - mean_plaquette**2)/N_bootstrap
            err_rect += np.abs(ensemble_of_rects[i]**2 - mean_rect**2)/N_bootstrap
        err_plaquette = np.sqrt(err_plaquette/N_bootstrap)
        err_rect = np.sqrt(err_rect/N_bootstrap)
...
wilson_plaquette, wilson_rect, err_plaquette, err_rect = test.bootstrap_plaquettes()
wilson_square = test.average_over_paths(test.wilson_rectangle, [2,2])

#wilson_plaquette = test.average_over_paths(test.wilson_rectangle)
#wilson_rect = test.average_over_paths(test.wilson_rectangle, [2,1])
#wilson_square = test.average_over_paths(test.wilson_rectangle, [2,2])
