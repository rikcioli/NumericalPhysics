...
class MyClass:
    ...
    def delta_energy_vs_t(self):
        ...
        #evaluate SDOM of bootstrap values of DeltaE
        err_deltaE = np.zeros(self.N_lat)
        for i in range(N_bootstrap_copies):
            err_deltaE += np.abs(ensemble_of_energies[i]**2 - mean_deltaE**2)/N_bootstrap_copies
        err_deltaE = np.sqrt(err_deltaE/N_bootstrap_copies)
